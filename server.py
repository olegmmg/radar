...
# ПРИОРИТЕТЫ: меньше = важнее (clear=4 может перебить любой через отдельную логику)
...
def _85(_86):
    _87 = _86.lower()
    print(f"🔍 Парсинг сообщения: {repr(_87[:200])}")  # <- добавить
    
    _88 = set()
    _89 = set()
    
    # 1. ТОЧНОЕ СОВПАДЕНИЕ - названия регионов
    for _90, _91 in _22.items():
        if _90.lower() in _87 and _91 not in _89:
            print(f"  Найден регион: {_91} по ключу {_90}")  # <- добавить
            _88.add(_91)
            _89.add(_91)
    
    
    # 2. ПОИСК ГОРОДОВ (г.Пермь, Пермь, город Пермь)
    _city_pattern = r'(?:г\.|город|города)\s*([А-Яа-яёЁ-]+)'
    _cities = R.findall(_city_pattern, _87)
    for city in _cities:
        for full_name, region in _22.items():
            if city.lower() == full_name.lower() or (len(city) > 2 and city.lower() in full_name.lower()):
                if region not in _89:
                    _88.add(region)
                    _89.add(region)
                    break
    
    # 3. ПОИСК РАЙОНОВ (Пермский район)
    _district_pattern = r'([А-Яа-яёЁ-]+)\s+район'
    _districts = R.findall(_district_pattern, _87)
    for district in _districts:
        for full_name, region in _22.items():
            if district.lower() in full_name.lower() or full_name.lower().startswith(district.lower()):
                if region not in _89:
                    _88.add(region)
                    _89.add(region)
                    break
    
    # 4. СКЛОНЕНИЯ (костромской области)
    ...
    # 5. ОБЛАСТИ В РОДИТЕЛЬНОМ ПАДЕЖЕ (Вологодской области)
...
def _108(_109, _110=None, _111="main", _112=None, _113=False):
    print(f"📨 ВХОДНОЕ СООБЩЕНИЕ [{_111}]: {repr(_109[:300])}")
    global _15, _16
    ...
    for _120 in _116:
        ...
        if _111 != "dpr" and not _113:
            # Для MAIN канала: отбой перебивает всё, иначе проверяем приоритет
            if _121 is not None:
                if _114 == "clear":
                    pass  # отбой применяется всегда
                elif _12.get(_114, 99) > _12.get(_121, 99):
                    continue  # новый статус менее важный - пропускаем
        # Для DPR канала: проверки приоритетов НЕТ - перебиваем любой статус

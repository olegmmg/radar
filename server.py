...
# ИСПРАВЛЕНО: убраны короткие ключи которые давали ложные срабатывания через подстроки.
# Ключи отсортированы от длинных к коротким чтобы более специфичные совпадения имели приоритет.
# Удалены: "Москва" (триггерил в @radarrussiia), "Псков" (в "Новопсков"),
# "Ульяновск" (в "Ульяновский район"), "Киров" (в "Киевский"), "Омская область" (в "Костромская")
# Вместо них используется поиск с границами слова (\b) в шаге 1.
...
# Ключи которые требуют точного совпадения слова (word boundary), а не поиска подстроки.
# Это короткие слова которые могут входить в состав других слов или названий.
_22_word_boundary = {
    ...
    # Многословные — тоже требуют точного совпадения
    ...
    # Названия областей которые являются подстрокой других областей:
    # "омская область" входит в "костромская область"
    # "томская область" входит в "костромская область"  
    # "рязанская область" входит в "белгородская..." нет, но лучше перестраховаться
...
def _78(_79):
    ...
    _80 = ["❗️ВНИМАНИЕ","Враг планирует", "за прошедшую", "Впервые регионы РФ подверглись массовым РАКЕТНЫМ атакам","создать телеграм каналы для оповещения граждан","Ищите свой регион и подписывайтесь","НЕ БУДУТ БЛОКИРОВАТЬ","Нет вашего региона","вакансия без опыта","Пятёрочка ищет людей","платят от","АВАНС","Удалять негативные отзывы","Ставить лайки под роликами","Сравнивать цены","24/7 (https://t.me/","Москва 24/7","Питер 24/7","Подписывайтесь","будет автоматически выдан АВАНС"]
    for _81 in _80:
...
def _region_match(_key, _text):
    """
    Проверяет наличие ключа в тексте.
    Для коротких/потенциально-конфликтных ключей используется поиск с границами слова (\b).
    Для длинных специфичных ключей (названия областей) — обычный поиск подстроки.
    """
    _key_lower = _key.lower()
    _text_lower = _text.lower()

    if _key in _22_word_boundary:
        # Ищем с границами слова чтобы "Псков" не находил "Новопсков",
        # "Омская" не находила "Костромская", "Киров" не находил "Киевский" и т.д.
        try:
            return bool(R.search(r'\b' + R.escape(_key_lower) + r'\b', _text_lower))
        except:
...
def _85(_86):
    ...
    # 1. ТОЧНОЕ СОВПАДЕНИЕ - с проверкой на границы слова для проблемных ключей
    for _90, _91 in _22.items():
        if _region_match(_90, _86) and _91 not in _89:
            _88.add(_91)
            _89.add(_91)

    # 2. ПОИСК ГОРОДОВ (г.Пермь, город Пермь) — только через явный префикс
    ...
    # 3. ПОИСК РАЙОНОВ (Пермский район) — только точное совпадение корня
    # ИСПРАВЛЕНО: теперь сопоставляем точно, а не через "in", чтобы "Ульяновский район"
    # не давал "Ульяновская область"
    _district_pattern = r'([А-Яа-яёЁ-]+)\s+район'
    _districts = R.findall(_district_pattern, _87)
    for district in _districts:
        district_lower = district.lower()
        for full_name, region in _22.items():
            # Только если название региона НАЧИНАЕТСЯ с корня района
            # и сам регион содержит слово "район" (т.е. это не область/край)
    ...
    # 4. СКЛОНЕНИЯ (костромской области → Костромская область)
    ...
    # 5. ОБЛАСТИ В ИМЕНИТЕЛЬНОМ ПАДЕЖЕ (Вологодская область)
    ...
    # 8. КОРОТКИЕ НАЗВАНИЯ РЕСПУБЛИК — с проверкой границ слова

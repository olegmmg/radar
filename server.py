...
def detect_status(text):
    ...
    # Атака БПЛА — фиксации, сбития, работа ПВО, тревога по БПЛА
    if any(w in t for w in [
        ...
        "тревога по бпла"  # ← было danger, стало attack
    ]):
        return "drone_attack"
    
    # Опасность по БПЛА (но НЕ тревога)
    if any(w in t for w in [
    ...
    # Сохраняется
    if "сохраняется" in t or "опасность сохраняется" in t:
        return "drone_danger"
    
    # Отбой
    if "отбой опасности" in t or "отбой по бпла" in t:
        if "опасность сохраняется" in t or "сохраняется" in t:
            return "drone_danger"
        return "clear"
